...
from django_mongo_browserid.auth import BrowserIDBackend, default_username_algo
...
class BrowserIDBackendTests(TestCase):
    ...
    @patch.object(settings, 'BROWSERID_CREATE_USER',
                  'django_mongo_browserid.tests.test_auth.new_user')
    @patch('django_mongo_browserid.tests.test_auth.new_user')
    ...
    @patch('django_mongo_browserid.signals.user_created')
    @patch.object(settings, 'BROWSERID_CREATE_USER', True)
