...
class mock_browserid(object):
    """
    Mocks django_mongo_browserid verification. Can be used as a context manager or
    as a decorator:

    with mock_browserid('a@b.com'):
        django_mongo_browserid.verify('random-token')  # = {'status': 'okay',
                                                 #    'email': 'a@b.com',
                                                 #    ...}

    @mock_browserid(None)
    def browserid_test():
        django_mongo_browserid.verify('random-token')  # = False
    """
    def __init__(self, email=None, audience=None):
        self.patcher = patch('django_mongo_browserid.base._verify_http_request')
        self.return_value = {
            u'audience': audience,
            u'email': email,
